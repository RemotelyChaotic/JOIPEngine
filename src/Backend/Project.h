...
public:
  CProject();
  explicit CProject(const CProject& other);
  ~CProject();
  ...
  Q_INVOKABLE void AddScene(const tspScene& sValue);
  Q_INVOKABLE void ClearScenes();
  Q_INVOKABLE void InsertScene(qint32 iIndex, const tspScene& sValue);
  Q_INVOKABLE qint32 NumScenes();
  Q_INVOKABLE void RemoveScene(qint32 iIndex);
  Q_INVOKABLE tspScene Scene(qint32 iIndex);
...
//----------------------------------------------------------------------------------------
//
typedef QSharedPointer<CProject> tspProject;
